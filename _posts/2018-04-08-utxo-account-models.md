...
tags: blockchain bitcoin ethereum utxo account
toc: true
...
在这里，我们会分别介绍两种不同区块链模型的实现方式以及优缺点，我们会分别以 Bitcoin 和 Ethereum 为例介绍 UTXO 模型和介绍账户余额模型。
...
在每一笔合法的交易中，所有的输入的 `value` 之和必须小于所有输出的 `value` 之和，这两者之间的差值就是矿工费：
